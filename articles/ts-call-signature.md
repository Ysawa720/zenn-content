...
:::message
関連する内部メソッドに `[[Construct]]` があります。これは `new` で呼ばれたときの動作を定義するものです。`function` 宣言の関数は `[[Call]]` と `[[Construct]]` の両方を持ちますが、アロー関数は `[[Call]]` だけを持ち `[[Construct]]` を持ちません。だからアロー関数は `new` できない（`new Bar()` が `TypeError: Bar is not a constructor` になる）のです。
:::
