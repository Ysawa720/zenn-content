...
一般的に、UIの遅延は以下のように体感されます（AI調べ）。

| 遅延 | 体感 |
|---|---|
| ~100ms | ほぼ気にならない |
| 100〜300ms | 「少しもたつく」と感じ始める |
| 300〜1000ms | 明らかに遅い・ストレスを感じる |
| 1000ms以上 | 「壊れてる？」と思う |

Supabase へのDB通信は一般的に 200〜600ms 程度のようです。電波が安定しているグラウンドなら 200ms 台に収まるかもしれませんが、試合中のWi-Fiや4G環境では 500ms 以上になることも想定されます。
つまり、「少しもたつく」から「明らかに遅い」の間をうろうろする状況です。投球のたびにこの遅延が発生すると、記録作業のテンポが崩れます。楽観的UIでタップ即反映にすることで、この問題を解消できます。
...
tap2のoptimisticは消えません。この再計算の過程で重複表示が起きることがありますが、**なぜ起きるかの詳細なメカニズム（ReactのLane管理と巻き戻し処理の関係）については勉強不足で、まだ正確に理解できていません**。ReactのLane・Fiber・useTransitionの内部実装まで踏み込む必要があるようです。
